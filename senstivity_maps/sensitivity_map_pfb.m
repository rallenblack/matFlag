...
% LO_freq = 1450;
% faxis = ((-249:250)*303.75e-3) + LO_freq; % freqs should be different
chansel = 1;
LO = 1450e6;
...
% e = source.e;
% f = source.f;
% x = a + b*log10(faxis./1e3) + c*log10(faxis./1e3).^2 + d*log10(faxis./1e3).^3 + e*log10(faxis./1e3).^4 + f*log10(faxis./1e3).^5;
% flux_density = 10.^x;
flux_density = 187;
...
for j = 1:length(off_tstamp)
    ...
    if ~exist(filename, 'file') || overwrite == 1
        [R, az_tmp, el_tmp, ~] = aggregate_banks_pfb(save_dir, ant_dir, tmp_stmp, -1);
...
for i = 1:length(on_tstamp)
    ...
    % Extract data and save
    if ~exist(filename, 'file') || overwrite == 1
        [R, az, el, ~] = aggregate_banks_pfb(save_dir, ant_dir, tmp_stmp, 10);
        save(filename, 'R', 'az', 'el');
    else
        load(filename);
    end
...
%                 Senstmp(t,b) = 2*kB*SNR(t,b)./(flux_density(b)*1e-26);
                Senstmp(t,b) = 2*kB*SNR(t,b)./(flux_density*1e-26);
            else
                Senstmp(t,b) = 0;
