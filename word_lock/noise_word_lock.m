...
%     figure();
%     plot(freqs/1e6, phi);
%     title(sprintf('Unwrapped Relative Phase, 1/%d', ref_ele));
%     xlabel('Frequency (MHz)');
%     ylabel('Phase, unwrapped (rad)');

    good_idx = find(~isnan(phi));
    F1 = [ones(length(good_idx),1), freqs(good_idx).'];
    phi1 = phi(good_idx);

    % solve for ramps
    a_b = F1\phi1;
    residual = norm(F1*a_b - phi1).^2;
