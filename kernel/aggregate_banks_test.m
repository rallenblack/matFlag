function [ R, my_az, my_el, info ] = aggregate_banks_test( save_dir, ant_dir, offstamp, onstamp, Nint )
...
%   40x40x500 matrix.
    ...
    chan _idx = [1:5, 101:105, 201:205, 301:305, 401:405];
    ...
    for i = 1:length(banks)
        filename = sprintf('%s/%s%s.fits', save_dir, offstamp, banks{i});
        fprintf('%c', banks{i});
    ...
    use_radec = 0;
    antoff_fits_file = sprintf('%s/%s.fits', ant_dir, offstamp);
    anton_fits_file = sprintf('%s/%s.fits', ant_dir, onstamp);
    [ant_dmjd, az_off, el_off] = get_antenna_positions_test(antoff_fits_file, anton_fits_file, use_radec);
