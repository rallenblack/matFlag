...
for i = 1:size(onoffs_7pt,1)
    session   = onoffs_7pt{i,1};
    on_scans  = onoffs_7pt{i,2};
    off_scans = onoffs_7pt{i,3};
    source    = onoffs_7pt{i,4};
    LO_freq   = onoffs_7pt{i,5};
    
    fprintf('Processing %s - scans %d-%d\n', session.session_name,...
                                             off_scans(1), off_scans(2));
                                         
    proj_str = session.session_name;
    save_dir = sprintf('%s/%s/BF', data_root, proj_str);
    
    
    [Tsys_eta_X, Tsys_eta_Y, freqs, wX, wY, maxSens_idx] = get_onoff_7pt_tsys(session,...
                                                             on_scans,...
                                                             off_scans,...
                                                             source,...
                                                             LO_freq);
    ...
    [~] = mkdir('figures');
    tsys_filename = sprintf('figures/%s_scans%d_%d_Xpol_tsys',...
        session.session_name, off_scans(1), off_scans(2));
    saveas(tsysX_fig, sprintf('%s.png', tsys_filename));
    saveas(tsysX_fig, sprintf('%s.pdf', tsys_filename));
    saveas(tsysX_fig, sprintf('%s.eps', tsys_filename));
    saveas(tsysX_fig, sprintf('%s.fig', tsys_filename), 'fig');
    ...
    % Save the Tsys spectrum data
    Tsys_eta = Tsys_eta_X;
    save(sprintf('%s.mat', tsys_filename), 'Tsys_eta', 'freqs', 'maxSens_idx');
